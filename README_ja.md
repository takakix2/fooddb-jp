...
```bash
# 鶏肉を検索
curl "https://fooddb.navii.online/foods/search/鶏"
...
**無料で始める:** [https://fooddb.navii.online/start](https://fooddb.navii.online/start) — キー不要の `mcp_config.json` をコピーして Free プラン（100リクエスト/日）で即開始。
...
Freeプランは APIキー不要です（[`/start`](https://fooddb.navii.online/start) で MCP 設定を取得）。上位プランは `/billing/checkout` から申し込みできます。
