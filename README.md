...
**Start free:** [https://fooddb.navii.online/start](https://fooddb.navii.online/start) — copy a ready-to-paste keyless `mcp_config.json` and start on the Free tier (no API key).
...
Free tier requires no API key — grab your MCP config at [`/start`](https://fooddb.navii.online/start). For higher limits, visit `/billing/checkout`.
